...
async function startServer() {
  ...
  // Bulk put menus (overwrite from spreadsheet)
  app.put('/api/menus', (req, res) => {
    const newProducts = req.body;
    if (!Array.isArray(newProducts)) {
      return res.status(400).json({ error: 'Data menu harus berupa array!' });
    }
    db.products = newProducts;
    writeLog(`Melakukan sinkronisasi katalog menu sebanyak ${newProducts.length} produk dari Google Sheets`, 'Admin');
    saveDB();
    res.json(db.products);
  });
